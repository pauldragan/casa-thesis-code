...
public class SWIMDefaultReactiveAdaptationManager extends SWIMAdaptationManagerBase {
	
	private static final Logger logger = LoggerFactory.getLogger(SWIMDefaultReactiveAdaptationManager.class);
	private static final StatsCollector collector = StatsCollector.getInstance();
	
	public SWIMDefaultReactiveAdaptationManager(CASABase casa) {
		super(casa);
	}
	...
	public SWIMAdaptation planAdaptation(SWIMReceivedStruct swimReceivedStruct) {
		...
		if (obs.avg_response_time > runtimeThreshold) {
			...
			double spareUtilization = obs.servers - obs.utilization;
			if (spareUtilization > 1.0) {
				...
				} else if (!obs.is_booting && obs.active_servers > 1) {
