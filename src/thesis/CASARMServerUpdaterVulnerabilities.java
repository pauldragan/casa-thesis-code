...
public class CASARMServerUpdaterVulnerabilities extends CASARMServerUpdaterBase{
    private static final Logger logger = LoggerFactory.getLogger(CASARMServerUpdaterVulnerabilities.class);
    
    protected static final String compNameBase = "Vulnerability_";
    
    @Override
    protected int determineServerNumberFromModel(EGraph model) {
		List<Match> matches = HenshinUtils.applyMatchRule(model, this.identifyStateRules.get("IdentifyVulnerabilityInstance"), engine);
		if (matches == null) {
			return 0;
		} else {
			return matches.size();
	...
	private void addServers(EGraph model, int oldServers, int newServers) {
		for (int server = oldServers + 1; server <= newServers; server += 1) {
			Rule rule = this.changeServerRules.get("AddVulnerability");
			String serverName = compNameBase + server;
			
			List<Pair<String,String>> params = new ArrayList<>();
			params.add(new ImmutablePair<String, String>("vulnerabilityName", serverName));
			// params.add(new ImmutablePair<String, String>("functionName", funcName));
			
			logger.info("Adding vulnerability=" + serverName);
	...
	private void removeServers(EGraph model, int oldServers, int newServers) {
		for (int server = oldServers; server > newServers; server -= 1) {
			Rule rule = this.changeServerRules.get("RemoveVulnerability");
			String serverName = compNameBase + server;
			...
			logger.info("Removing vulnerability=" + serverName);
			HenshinUtils.applyModelChangeRule(model, rule, engine, params);
		}
	}
