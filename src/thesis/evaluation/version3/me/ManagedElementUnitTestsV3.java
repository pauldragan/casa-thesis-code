...
public final class ManagedElementUnitTestsV3 {
    private ManagedElementUnitTestsV3() {
    }

    public static void runAll() {
        TestSupportV3.run("me.samplesFunctionRelationsLocallyPerFunction",
                ManagedElementUnitTestsV3::samplesFunctionRelationsLocallyPerFunction);
        TestSupportV3.run("me.baselineConsentMatchesUnionOfProcessingFunctionPurposesPerPd",
                ManagedElementUnitTestsV3::baselineConsentMatchesUnionOfProcessingFunctionPurposesPerPd);
    }
    ...
    private static void samplesFunctionRelationsLocallyPerFunction() {
        ...
        Set<Integer> realizedByCounts = me.functions.values().stream()
                .map(fn -> fn.realizedByComponentIds.size())
                .collect(Collectors.toSet());
        Set<Integer> processCounts = me.functions.values().stream()
                .map(fn -> fn.processesPersonalDataIds.size())
                .collect(Collectors.toSet());
        Set<Integer> purposeCounts = me.functions.values().stream()
                .map(fn -> fn.hasPurposeIds.size())
                .collect(Collectors.toSet());

        TestSupportV3.check(realizedByCounts.size() > 1, "ME generation should sample realizedBy locally per function");
        TestSupportV3.check(processCounts.size() > 1, "ME generation should sample processes locally per function");
        TestSupportV3.check(purposeCounts.size() > 1, "ME generation should sample hasPurpose locally per function");
    }
    ...
    private static void baselineConsentMatchesUnionOfProcessingFunctionPurposesPerPd() {
        ...
        for (ManagedElementInstance.PersonalDataNode pd : me.personalData.values()) {
            ...
            TestSupportV3.equals(expectedPurposes, actualPurposes,
                    "Baseline consents should match processing purposes for PD " + pd.id);
        }
    }
